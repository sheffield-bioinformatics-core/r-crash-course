...
As with `dplyr`, we won't have time to cover all details of `ggplot2`. This is however a useful [cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf)
...
Lets say that we want to explore the relationship between GDP and Life Expectancy; Do richer / poorer countries have higher / lower life expectancy. A sensible choice of plot would be a *scatter plot* with gdp on the x-axis and life expectancy on the y-axis.
...
Our plots are a bit dreary at the moment, but we can add colour to them by mapping other aesthetics from the `gapminder` data frame.
...
```{r}
ggplot(gapminder, aes(x = gdpPercap, y=lifeExp,shape=continent,col=as.factor(year))) + geom_point()
```


```{r}
ggplot(gapminder, aes(x = as.factor(year), y=gdpPercap,fill=as.factor(year))) + geom_boxplot()
```

There are lots of possibilities. It is easy to get carried-away!
```{r}
...
Rather than putting too much information onto a single plot, we can use the technique of facetting to create a grid of plots

```{r}
ggplot(gapminder, aes(x = gdpPercap, y=lifeExp,col=continent,size=pop)) + geom_point() + facet_wrap(~year)

```

```{r}
filter(gapminder, year %in% c(1982,1987,1992,1997)) %>% 
  ggplot(aes(x = as.factor(year), y=gdpPercap,fill=as.factor(year))) + geom_boxplot() + facet_wrap(~continent) + scale_fill_brewer(palette = "Set2")
```


```{r}
ggplot(gapminder,aes(x=year,y=lifeExp,col=country)) + geom_line() + facet_wrap(~continent) + theme(legend.position = "none")
```


```{r}

ggplot(gapminder, aes(x = as.factor(year), y=gdpPercap,fill=as.factor(year))) + geom_boxplot() + facet_wrap(~continent) 
```
